...
| `recommendation/` | 推荐结果 | popular/content/itemcf/usercf/hybrid 模型 + 全量热度分 + 全体活跃用户 Top-K + 运行记录 + 权重实验 + 多算法对比（Phase 11-14） |
...
- 权重实验（Phase 12）：`python scripts/run_weight_experiment.py` 读取 `processed/`，输出到 `recommendation/`（`weight_experiment.csv` / `weight_experiment.json`）。要点：对实验 A（1/2/3/4/5）、B（1/2/4/6/8）、C（1/2/3/5/10）在严格时间切分（历史→train、未来→test，推荐只用 train 信息）上做离线评估，指标含 Precision@K/Recall@K/F1@K/HitRate@K/NDCG@K/Coverage（开发文档第 36 节）；最优权重依据离线实验选择（NDCG@10 最高，并列时看 Recall@10），不以主观判断为依据（开发文档第 49.10 节）。本次真实数据结论：三种权重 NDCG@10 几乎持平，A（默认 1/2/3/4/5）最优。
- Content-Based（Phase 13）：`python scripts/run_content.py` 读取 `processed/`，输出到 `recommendation/`（`content_model.joblib` / `content_recommendations.csv` / `content_meta.json`）。要点：商品内容向量由 category（叶子分类 one-hot）/ brand（one-hot）/ price_range（qcut 价格分箱 one-hot）/ item tags（商品名分词 TF-IDF、去品牌 token）构造（开发文档第 49.11 节 / 35.4 节）；推荐流程=用户历史商品（种子，行为权重×时间衰减）→ 内容余弦相似商品 → 分数累加 → 过滤（已购买/下架/不存在/重复）→ Top-K；冷启动：新商品（无行为）也可被内容召回，新用户回退全局热门兜底（开发文档第 35.6 节）。
- Hybrid（Phase 14）：`python scripts/run_hybrid.py` 读取 `processed/`，输出到 `recommendation/`（`hybrid_model.joblib` / `hybrid_recommendations.csv` / `hybrid_meta.json` / `algo_comparison.csv` / `algo_comparison.json`）。要点：统一 ItemCF + UserCF + Popular + Content 四路召回（开发文档第 49.12 节 / 35.5 节），各分量在候选商品上先归一化到 [0,1] 再按权重线性融合 `HybridScore = w1*ItemCF + w2*UserCF + w3*Popular + w4*Content`；融合权重可配置（`REC_HYBRID_WEIGHTS`，JSON 或 `itemcf:0.25,...`）；Item-CF 用 user-item 加权矩阵（pv=1..buy=5）算 item-item 余弦，User-CF 算 user-user 余弦取 Top-N 相似用户（`REC_N_NEIGHBORS` 默认 50）加权；`algo_comparison.json` 记录 popular/itemcf/usercf/content/hybrid 的离线时间切分指标对比与结论（必须基于评估指标，若 Hybrid 不优于 baseline 不得强行声称更好）。本次真实数据结论：Hybrid NDCG@10=0.0085 优于 Popular baseline 0.0047（基于离线指标）。
